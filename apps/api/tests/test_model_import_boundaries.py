...
def test_leaf_models_do_not_load_core_models_or_registry() -> None:
    completed = _run_cold_import(
        "import sys; "
        "from mist_service.deliverable_model import Deliverable; "
        "from mist_service.feedback_model import Feedback; "
        "from mist_service.outbox_model import WorkflowOutbox; "
        ...
        "assert 'mist_service.models' not in sys.modules; "
        "assert 'mist_service.model_registry' not in sys.modules"
    )
    assert completed.returncode == 0, completed.stderr
...
def test_core_compatibility_exports_do_not_load_registry() -> None:
    completed = _run_cold_import(
        "import sys; "
        "from mist_service.models import Deliverable, Feedback, WorkflowOutbox; "
        "assert Deliverable.__tablename__ == 'deliverables'; "
        "assert Feedback.__tablename__ == 'feedback'; "
        "assert WorkflowOutbox.__tablename__ == 'workflow_outbox'; "
        "assert 'mist_service.model_registry' not in sys.modules"
