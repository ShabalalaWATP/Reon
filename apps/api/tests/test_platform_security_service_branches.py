...
def test_service_rejects_short_pseudonym_keys() -> None:
    with pytest.raises(ValueError, match="at least 32 bytes"):
        PlatformSecurityService(repository(), pseudonym_key=b"short")


@pytest.mark.asyncio
async def test_classification_mutation_checks_role_and_version() -> None:
    repo = repository()
    service = PlatformSecurityService(repo)
...
async def test_assistance_budget_enforces_each_limit_and_records_allowed_work() -> None:
    repo = repository()
    service = PlatformSecurityService(repo, pseudonym_key=b"x" * 32)
    ...
    repo.add_attempt.return_value = SimpleNamespace(id=attempt_id)
    assert (
        await service.request_password_assistance(
...
async def test_pending_assistance_handles_indexing_key_rotation_and_empty_queue() -> (
    ...
    repo = repository()
    service = PlatformSecurityService(repo, pseudonym_key_id="current")
    ...
    attempt = SimpleNamespace(email_key_id="old", email_hash="hash")
    repo.pending_attempt.return_value = attempt
    assert await service.process_pending_password_assistance() is True
    repo.retry_attempt.assert_called_once()


@pytest.mark.asyncio
async def test_assistance_indexing_and_direct_processing_cover_match_paths() -> None:
    repo = repository()
    first = SimpleNamespace(email=" One@example.test ")
    second = SimpleNamespace(email="two@example.test")
    ...
    assert first.assistance_email_key_id == second.assistance_email_key_id == "current"
...
async def test_pending_assistance_completes_matches_and_retries_failures() -> None:
    ...
    await service.process_pending_password_assistance()
    repo.retry_attempt.assert_called()
