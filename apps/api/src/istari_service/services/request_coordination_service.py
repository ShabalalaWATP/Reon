...
class RequestCoordinationService:
    ...
    async def post_message(
        ...
        if (
            ...
            raise InvalidAction(
                "Customers can send coordination to the current owner."
            )
        label = (
    ...
    async def request_return(
        ...
        target = await self._repository.route_target(
            request.id, command.target_unit_id
        )
        ...
        event = await append_request_event(
            ...
            message=(
                f"Return to {target[0].name} requested: {command.reason}"
            ),
            prior_status=request.status,
            next_status=request.status,
    ...
    @staticmethod
    def _event_view(event: object, actor_name: str) -> TrackedRequestEvent:
