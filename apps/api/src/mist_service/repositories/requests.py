...
from mist_service.repositories.request_cancellation import cancel_request
from mist_service.repositories.request_customer import RequestCustomerRepositoryMixin
...
from mist_service.repositories.request_scope import scoped_request
from mist_service.repositories.request_views import (
    build_request_detail,
)
from mist_service.request_event_audience import RequestEventAudience
from mist_service.request_search_projection import new_search_document
...
class SqlAlchemyRequestRepository(RequestCustomerRepositoryMixin):
    ...
    async def _request_for_submission_key(
        self, submission_key: UUID
    ) -> ServiceRequest | None:
        existing: ServiceRequest | None = await self._session.scalar(
            select(ServiceRequest).where(
                ServiceRequest.submission_key == submission_key
            )
        )
        return existing

    async def _original_submission(
        self, existing: ServiceRequest, actor: Actor
    ) -> RequestDetail:
        if existing.requester_id != actor.id:
            raise ObjectNotFound()
        return await self.get_detail(existing.id, reveal_unreleased_deliverable=False)
    ...
    async def create(
        ...
        existing = await self._request_for_submission_key(command.submission_key)
        if existing is not None:
            return await self._original_submission(existing, actor)
        request_id = uuid4()
        ...
        try:
            ...
            # A concurrent submission won the unique submission key. Answer with
            # the request it created so a retry stays idempotent.
            duplicate = await self._request_for_submission_key(command.submission_key)
            if duplicate is None:
                raise
            return await self._original_submission(duplicate, actor)
