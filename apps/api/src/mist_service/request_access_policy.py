...
LOADED_UNDER_POOL_SCOPE = True
"""Evidence marker for records fetched through an actor-scoped repository query.

The scoped queries in ``repositories.request_scope`` and ``repositories.work_scope``
already require live route or QC membership, so any record they return is
pool-scoped by construction. Passing this marker instead of a bare literal keeps
every reliance on that guarantee greppable, and leaves callers that never checked
membership on the denying default.
"""

POOL_SCOPED_ROLES = frozenset(
    {
...
def is_object_scoped(
    ...
    *,
    ...
    Customer, Team Manager and Team Analyst scope is decided entirely here from
    the request itself. The shared-pool roles are instead scoped by live unit
    membership, which only the persistence boundary can evaluate, so the caller
    must supply the outcome of that check. Omitting the evidence denies access
    rather than granting it on the role alone.
    """
    ...
    return pool_membership_verified and actor.role in POOL_SCOPED_ROLES
...
def can_access_work(
    ...
    *,
    ...
    return has_stage_role(actor, request) and is_object_scoped(
...
def decide_request_access(
    ...
    *,
    ...
    visible = _request_is_visible(
        actor, request, pool_membership_verified=pool_membership_verified
    )
    if operation is RequestOperation.VIEW:
        return ALLOW if visible else deny(PolicyDenial.OBJECT_SCOPE)
    if operation in {
