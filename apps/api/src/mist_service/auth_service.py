...
from mist_service.auth_ports import AuthRepository as AuthRepository
from mist_service.auth_primitives import (
...
class AuthService:
    ...
    async def elevate(self, session: SessionRecord, password: str) -> datetime:
        ...
        if not valid or locked:
            if locked:
                ...
                raise AuthenticationFailed()
            await self._record_step_up_failure(account, now)
            await self._record_security(
                "STEP_UP",
                SecurityOutcome.FAILURE,
                "INVALID_CREDENTIALS",
    ...
    async def _record_step_up_failure(
        self, account: AccountRecord, now: datetime
    ) -> None:
        await self._repository.record_step_up_failure(
            account,
            now=now,
