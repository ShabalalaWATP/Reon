...
class BoardPackagePolicy:
    def __init__(
        self, planning: BoardPlanningReadPort, queries: BoardQueryPort
    ) -> None:
    ...
    async def authorise_create(
        ...
        members = await self._planning.current_member_ids(team_id)
        ...
        if actor.role is UserRole.DELIVERY_TEAM_LEAD:
            ...
            await authorise_board_manager(
                self._planning, actor, team_id, command.grant_id
            )
            return
        require(
    ...
    async def validate_links(
        ...
        require(
            await self._planning.package_ids_in_team(team_id, dependencies)
            == dependencies,
            BoardItemNotFound(),
        )
        ...
        if command.iteration_id:
            require(
                await self._planning.iteration_in_team(team_id, command.iteration_id),
                BoardItemNotFound(),
            )
    ...
    async def enforce_wip(
        ...
        if limit is None:
            return
        count = await self._queries.column_count(
            team_id,
            target_column,
    ...
    async def _validate_request_link(
        ...
        if request_id is None:
            return
        require(
            await self._planning.request_belongs_to_team(team_id, request_id),
            BoardItemNotFound(),
        )
        requester_id = await self._planning.request_requester_id(request_id)
