...
describe("authentication session lifecycle", () => {
  it("propagates only valid cross-tab sign-out events", async () => {
    ...
    act(() => window.dispatchEvent(new StorageEvent("storage", { key: "istari:auth-state", newValue: "signed-in:123" })));
    expect(screen.getByRole("heading", { name: "My requests" })).toBeInTheDocument();
    act(() => window.dispatchEvent(new StorageEvent("storage", { key: "istari:auth-state", newValue: "signed-out:124" })));
  ...
  it("enforces absolute expiry locally even when logout fails", async () => {
    ...
    mockFetch((url) => url.pathname.endsWith("/auth/me") ? json(expired) : Promise.reject(new Error("offline")));
    renderApp("/requests");
    expect(await screen.findByRole("heading", { name: "Sign in" })).toBeInTheDocument();
  });
  ...
  it("records only visible, unthrottled activity and clears on a 401", async () => {
    ...
    mockFetch((url) => {
      ...
      if (url.pathname.endsWith("/auth/activity")) { activityCalls += 1; return json({ detail: "failed" }, activityStatus); }
      return json({ items: [] });
    });
    renderApp("/requests");
    expect(await screen.findByRole("heading", { name: "My requests" })).toBeInTheDocument();
  ...
  it("removes elevation when its timer fires", async () => {
    const elevated = { ...requesterSession, elevatedUntil: new Date(Date.now() + 5_000).toISOString() };
    const nativeSetTimeout = window.setTimeout.bind(window);
    let expire: (() => void) | undefined;
    vi.spyOn(window, "setTimeout").mockImplementation((handler, timeout, ...arguments_) => {
      if (typeof handler === "function" && Number(timeout) >= 4_000 && Number(timeout) <= 6_000) expire = () => handler();
    ...
    render(<TestProviders><AuthProvider><ElevationProbe /></AuthProvider></TestProviders>);
    ...
    act(() => window.dispatchEvent(new StorageEvent("storage", {
      key: "istari:auth-state", newValue: "signed-out:after-elevation",
    })));
    act(() => expire?.());
    expect(await screen.findByText("Not elevated")).toBeInTheDocument();
  });
  ...
  it("keeps an already anonymous session private when logout is requested", async () => {
    ...
    render(<TestProviders><AuthProvider><AnonymousLogoutProbe /></AuthProvider></TestProviders>);
...
function AnonymousLogoutProbe() {
  ...
  return (
    <><span>{status === "anonymous" ? "Anonymous" : "Loading"}</span>
      <button type="button" onClick={() => void logout()}>Clear local session</button></>
  );
}
