...
const emptyActions = {
  items: [], savedViews: [], nextCursor: null,
...
const overviewScope = {
  id: "scope-overview", unitId: "unit-overview", name: "Platform service", kind: "ROOT",
  includeDescendants: true, units: [],
};
const emptyStatistics = { summary: [], dueRisk: [] };
const overviewWorkspace = {
  teamId: "crioc", teamCode: "CRIOC", teamName: "CRIOC", unitKind: "ROOT",
  workspacePosition: "MEMBER", grantId: null, permissions: [],
};
...
describe("staff landing policy", () => {
  it("opens the Administrator Home without loading request content", async () => {
    ...
    expect(screen.getAllByRole("link", { name: /User accounts/ })[0]).toHaveAttribute("href", "/admin/users");
  ...
  it("redirects staff away from Customer routes to Home", async () => {
    ...
    expect(screen.getAllByRole("link", { name: /CRIOC workspace/ })[0]).toHaveAttribute("href", "/teams/crioc/overview");
  });
  ...
  it("keeps staff Home available when optional dashboards are disabled", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Welcome, Scott" })).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "Your workload" })).toHaveTextContent("Needs your action0");
