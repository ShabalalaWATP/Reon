...
export function RelatedRecordPanel({ csrfToken, userId, workItemId }: Props) {
  ...
  return (
    <section aria-labelledby="related-record-title" className="related-record-panel">
      <div className="section-heading"><span>Request comparison</span><h2 id="related-record-title">Previous request matches</h2></div>
      <p>All submitted fields are compared with earlier requests you are authorised to view. Scores help focus a human review and never change the route.</p>
      ...
      <form className="related-record-search" onSubmit={submitSearch} role="search">
        <label className="form-field"><span>Search all submitted fields</span><input minLength={2} onChange={(event) => setDraft(event.target.value)} placeholder="Add terms or a request reference" required value={draft} /></label>
      ...
      {results.data?.items.length ? <MatchResults items={results.data.items} onSelect={(item) => { setSelected(item); if (!item.productAvailable && linkType === "EXISTING_OUTPUT") setLinkType("RELATED_REQUEST"); }} selectedId={selected?.id} /> : null}
      {selected && links.data ? (
