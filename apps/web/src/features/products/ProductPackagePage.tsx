...
export function ProductPackagePage() {
  ...
  const packageQueryKey = queryKeys.productPackage(packageId ?? "");
  ...
  if (query.isError)
    return (
      <PageState
        ...
        kind="error"
        title="Product package could not be loaded"
      >
        It may have changed or you may no longer have access.
      </PageState>
    );
  const productPackage = query.data;
  const editable =
    session!.user.role === "DELIVERY_SPECIALIST" && productPackage.status === "DRAFT";
  return (
    <main className="page-stack product-package-page">
      <Link
        ...
        to={
          session!.user.role === "DELIVERY_SPECIALIST"
            ? "/delivery/my-work"
            : session!.user.role === "DELIVERY_TEAM_LEAD"
              ? "/delivery/team"
              : "/quality-release"
        }
      >
      ...
      <header className="detail-heading product-package-heading">
        <div>
          <span className="mono-ref">
            {productPackage.requestReference} · package v{productPackage.packageVersion}
          </span>
          <h1>{productPackage.requestTitle}</h1>
          <p>Assemble, review and disseminate one immutable release version.</p>
        </div>
        <span className={`product-state product-state--${productPackage.status.toLowerCase()}`}>
          {packageStatusLabels[productPackage.status]}
        </span>
      </header>
      <div className="product-workspace">
        <div>
          {editable ? (
            <PackageBuilder onChanged={() => query.refetch()} productPackage={productPackage} />
          ) : (
            <section className="product-builder" aria-labelledby="package-contents-title">
        ...
        <PackageReviewPanel
          onChanged={(updated) => client.setQueryData(packageQueryKey, updated)}
...
function CreatePackage() {
  ...
  const create = useMutation({
    mutationFn: (input: { requestId: string; expectedVersion: number }) =>
      productApi.createPackage({ ...input, idempotencyKey: newProductKey() }, session!.csrfToken),
    ...
    <main className="page-stack page-stack--narrow">
      <header className="page-heading">
        <div>
          ...
          <h1>Start release package</h1>
          <p>Create a version against the exact assigned request record.</p>
        </div>
      </header>
      <form className="product-create-form" onSubmit={submit} noValidate>
        <label className="form-field">
          <span>Request identifier</span>
          <input defaultValue={search.get("requestId") ?? ""} name="requestId" />
        </label>
        <label className="form-field">
          <span>Request version</span>
          <input
            defaultValue={search.get("version") ?? ""}
            min={1}
        ...
        <button className="button button--primary" disabled={create.isPending} type="submit">
          {create.isPending ? "Creating package…" : "Create release package"}
        </button>
      </form>
    </main>
  );
