...
export function StaffProductAction({ requestId, requestVersion, stage }: { requestId: string; requestVersion: number; stage: WorkStage }) {
  const { capabilities, isPending } = useCapabilities();
  if (isPending || !capabilities.products) return null;
  return <EnabledStaffProductAction requestId={requestId} requestVersion={requestVersion} stage={stage} />;
}

function EnabledStaffProductAction({ requestId, requestVersion, stage }: { requestId: string; requestVersion: number; stage: WorkStage }) {
  const { session } = useAuth();
  const role = session!.user.role;
  const relevant = role in stagesByRole && stagesByRole[role as keyof typeof stagesByRole].includes(stage as never);
  const productPackage = useQuery({
    enabled: relevant,
    queryFn: () => productApi.packageForRequest(requestId),
    queryKey: ["protected", session!.user.id, "request-product-package", requestId],
  });
