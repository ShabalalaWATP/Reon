...
export function WorkPackageFields({
  ...
  const contributors = mode === "create"
    ? current.filter((member) => member.role === "DELIVERY_SPECIALIST")
    : current;
  const label = (create: string, edit: string) => mode === "edit" ? edit : create;
  const update = <K extends keyof WorkPackageFormValue>(
    name: K,
    next: WorkPackageFormValue[K],
  ...
  return (
    <>
      <Field label={label("Title", "Edit title")}>
        <input maxLength={160} minLength={3} onChange={(event) => update("title", event.target.value)} required value={value.title} />
      </Field>
      <Field label={label("Description", "Edit description")}>
        <textarea maxLength={4000} onChange={(event) => update("description", event.target.value)} required rows={mode === "create" ? 4 : undefined} value={value.description} />
      </Field>
      <div className="planning-form-grid">
        <Field label={label("Owner", "Edit owner")}>
          <select disabled={!canChooseOwner} onChange={(event) => update("ownerUserId", event.target.value)} required value={value.ownerUserId}>
      ...
      <div className="planning-form-grid">
        <Field label={label("Estimate points", "Edit estimate points")}>
          <input max={100} min={1} onChange={(event) => update("estimatePoints", Number(event.target.value))} required type="number" value={value.estimatePoints} />
        </Field>
        <Field label={label("Remaining effort (minutes)", "Edit remaining minutes")}>
          <input max={100000} min={0} onChange={(event) => update("remainingEffortMinutes", Number(event.target.value))} required type="number" value={value.remainingEffortMinutes} />
        </Field>
      </div>
      <div className="planning-form-grid">
        <Field label={label("Due date", "Edit due date")}>
          <input onChange={(event) => update("dueOn", event.target.value)} required type="date" value={value.dueOn} />
        </Field>
        <Field label={label("Priority", "Edit priority")}>
          <select onChange={(event) => update("priority", event.target.value as WorkPackagePriority)} required value={value.priority}>
      ...
      <Field label={label("Blockers or none", "Edit blockers or none")}>
        <textarea maxLength={4000} onChange={(event) => update("blockers", event.target.value)} required rows={mode === "create" ? 3 : undefined} value={value.blockers} />
      </Field>
      <Field label={label("Acceptance criteria", "Edit acceptance criteria")}>
        <textarea maxLength={4000} onChange={(event) => update("acceptanceCriteria", event.target.value)} required rows={mode === "create" ? 3 : undefined} value={value.acceptanceCriteria} />
      </Field>
      <div className="planning-form-grid">
        <Optional label={label("Linked request ID", "Edit linked request ID")}>
          <input onChange={(event) => update("linkedRequestId", event.target.value)} placeholder="UUID" value={value.linkedRequestId} />
        </Optional>
        <Optional label={label("Iteration", "Edit iteration")}>
          <select onChange={(event) => update("iterationId", event.target.value)} value={value.iterationId}>
            <option value="">No iteration</option>
            {iterations.filter((iteration) => iteration.status !== "CLOSED" || iteration.id === value.iterationId).map((iteration) => <option key={iteration.id} value={iteration.id}>{iteration.name}</option>)}
          </select>
        </Optional>
      </div>
      {mode === "edit" ? (
        <Optional label="Edit dependencies">
          <select multiple onChange={(event) => update("dependencyIds", Array.from(event.target.selectedOptions, (option) => option.value))} value={value.dependencyIds}>
            {dependencies.map((dependency) => <option key={dependency.id} value={dependency.id}>{dependency.title}</option>)}
          </select>
        </Optional>
      ) : null}
    </>
