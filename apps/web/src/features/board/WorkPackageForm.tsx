...
export function WorkPackageForm({
  ...
  return (
    <section className="package-form-panel">
      <header>
        <span>Analyst team planning</span>
        <h2>Create internal card</h2>
        <p>
          This card stays on the team’s internal Kanban and does not change the Customer request
          workflow.
        </p>
      </header>
      <form
      ...
      >
        ...
        <button className="button button--primary" disabled={mutation.isPending} type="submit">
          {mutation.isPending ? "Creating…" : "Add card to Kanban"}
        </button>
      </form>
    </section>
  );
...
function message(error: Error) {
  return error instanceof ApiError
    ? error.message
    : error.message || "The work package could not be created.";
}
