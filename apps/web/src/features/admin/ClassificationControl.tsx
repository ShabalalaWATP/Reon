...
} from "../../lib/api/platformSecurityClient";
import { isSessionElevated, useAuth } from "../../lib/auth/AuthProvider";
import {
  platformClassificationKey,
  usePlatformClassification,
} from "../../lib/platform/usePlatformClassification";
    ...
    <section className="classification-control" aria-labelledby="classification-control-title">
      ...
      {setting.isError ? (
        <p className="form-banner form-banner--error" role="alert">
          The classification setting could not be loaded.
        </p>
      ) : (
        <form onSubmit={submit}>
          <label className="form-field">
            ...
            <select
              disabled={!elevated || setting.isPending || update.isPending}
              onChange={(event) => setSelected(event.target.value as PlatformClassification)}
              value={selected}
            >
              {options.map((value) => (
          ...
          <button
            ...
            disabled={
              ...
              setting.isPending ||
              update.isPending ||
              selected === setting.data?.classification
            }
            type="submit"
          >
      ...
      {update.isSuccess ? (
        <p className="form-banner form-banner--success" role="status">
          Classification updated for every workspace.
        </p>
      ) : null}
      {update.isError ? (
        <p className="form-banner form-banner--error" role="alert">
          The classification changed elsewhere or could not be saved. Refresh and try again.
        </p>
      ) : null}
    </section>
  );
