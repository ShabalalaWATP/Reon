...
export function AdminUsersPage() {
  ...
  const users = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => api.adminUsers(query, pageParam ?? undefined),
  ...
  if (users.isError)
    return (
      <PageState
        action={
          <button className="button" onClick={() => void users.refetch()}>
            Try again
          </button>
        }
        kind="error"
    ...
    <main className="page-stack admin-users-page">
      <header className="page-heading">
        <div>
          ...
          <h1>User accounts</h1>
          <p>
            Maintain synthetic account metadata and organisation access. Request content is not
            available here.
          </p>
        </div>
      ...
      <dl className="admin-summary" aria-label="Account summary">
        <div>
          <dt>Active loaded</dt>
          <dd>{active}</dd>
        </div>
        <div>
          <dt>Inactive loaded</dt>
          <dd>{items.length - active}</dd>
        </div>
        <div>
          <dt>Total loaded</dt>
          <dd>{items.length}</dd>
      ...
      <section aria-labelledby="account-register-title">
        <div className="admin-register-heading">
          <div className="section-heading">
            ...
            <h2 id="account-register-title">Account register</h2>
          </div>
          <form className="admin-search" onSubmit={submitSearch} role="search">
            <label className="form-field">
              ...
              <input
                ...
                placeholder="Name, account, role or scope"
                value={draft}
              />
            </label>
        ...
        {items.length ? (
          ...
          <PageState
            kind="empty"
            title={query ? "No accounts match this search" : "No user accounts"}
          >
            {query
              ? "Clear the search or try another term."
              : "Create the first synthetic account."}
          </PageState>
        )}
      </section>
    </main>
