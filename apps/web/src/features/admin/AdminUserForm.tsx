...
export function AdminUserForm({ disabled, onSubmit, pending = false, units, user }: Props) {
  ...
  return (
    <form className="admin-user-form" noValidate onSubmit={(event) => void handleSubmit(onSubmit)(event)}>
      <fieldset><legend>Identity</legend><label className="form-field"><span>Username</span><input readOnly value={user?.username ?? "Assigned after creation"} /></label><label className="form-field"><span>Display name</span><input aria-invalid={Boolean(errors.displayName)} {...register("displayName")} />{errors.displayName ? <small role="alert">{errors.displayName.message}</small> : null}</label></fieldset>
      ...
      <fieldset><legend>Organisation membership</legend>{options.length ? <div className="admin-membership-list">{options.map((unit) => <label key={unit.id}><input type="checkbox" value={unit.id} {...register("organisationUnitIds")} /><span><strong>{unit.name}</strong><small>{unit.code}</small></span></label>)}</div> : <p className="inline-empty">This role does not require an organisation membership.</p>}{errors.organisationUnitIds ? <p className="field-error" role="alert">{errors.organisationUnitIds.message}</p> : null}</fieldset>
...
function valuesFor(user?: AdminUser): Values {
  return user ? { displayName: user.displayName, role: user.role, scope: user.scope, organisationUnitIds: user.memberships.map((membership) => membership.organisationUnitId) } : { displayName: "", role: "REQUESTER", scope: "", organisationUnitIds: [] };
}
