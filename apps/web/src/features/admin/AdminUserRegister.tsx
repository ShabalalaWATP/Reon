...
export function AdminUserRegister({ users }: { users: AdminUser[] }) {
  return (
    <div className="admin-table-scroll" role="region" aria-label="User account register" tabIndex={0}>
      <table className="admin-table">
        <thead><tr><th>Account</th><th>Name</th><th>Role</th><th>Scope</th><th>Memberships</th><th>Status</th><th><span className="sr-only">Action</span></th></tr></thead>
        <tbody>
          {users.map((user) => (
            <tr key={user.id}>
