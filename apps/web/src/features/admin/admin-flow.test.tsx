...
describe("platform administrator user management", () => {
  ...
  it("lists, searches and clears account metadata without request content", async () => {
    ...
    mockFetch((url) => {
      ...
      if (url.pathname.endsWith("/admin/users"))
        return json({
          items: url.searchParams.get("query")
            ? []
            : [
                {
                  ...adminManagedUser,
                  ...
                  memberships: [
                    {
                      organisationUnitId: organisationUnit("SSG_TEAM").id,
                      organisationUnitName: "SSG Team",
                      organisationUnitKind: "TEAM",
                    },
                  ],
                },
                {
    ...
    expect(
      await screen.findByRole("heading", { name: "No accounts match this search" }),
    ).toBeInTheDocument();
  ...
  it("applies the global classification with optimistic version protection", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Platform classification" }),
    ).toBeInTheDocument();
    ...
    expect(await screen.findByText("Classification updated for every workspace.")).toHaveAttribute(
      "role",
      "status",
    );
    expect(
      screen.getByRole("note", { name: "Security classification: OFFICIAL-SENSITIVE" }),
    ).toBeInTheDocument();
  ...
  it("recovers from a list error and reports an empty register", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "User accounts could not be loaded" }),
    ).toBeInTheDocument();
    fail = false;
  ...
  it("creates a generated account with compatible team membership", async () => {
    ...
    const saved = {
      ...
      memberships: [
        {
          organisationUnitId: organisationUnit("CEDAR_TEAM").id,
          organisationUnitName: "Cedar Team",
          organisationUnitKind: "TEAM" as const,
    ...
    mockFetch(async (url, init) => {
      ...
      if (url.pathname.endsWith("/admin/users") && init.method === "POST") {
    ...
    expect(screen.getByText(/Account created\. Username:/)).toHaveTextContent(
      "Account created. Username: admin100",
    );
    expect(screen.getByDisplayValue("admin100")).toBeInTheDocument();
    expect(posted).toMatchObject({
      displayName: "Billy Gilmour",
      role: "DELIVERY_SPECIALIST",
      organisationUnitIds: [organisationUnit("CEDAR_TEAM").id],
    });
    expect(await axe(view.container)).toHaveNoViolations();
  });
  ...
  it("edits and reversibly deactivates an account with version checks", async () => {
    ...
    mockFetch(async (url, init) => {
      if (url.pathname.endsWith("/auth/me")) return json(adminSession);
      if (url.pathname.endsWith("/organisation/units")) return json({ items: organisationUnits });
      if (url.pathname.endsWith(`/admin/users/${managed.id}/status`)) {
        const body = JSON.parse(String(init.body));
        bodies.push(body);
        managed = { ...managed, isActive: body.isActive, version: managed.version + 1 };
        return json(managed);
      }
      if (url.pathname.endsWith(`/admin/users/${managed.id}`) && init.method === "PATCH") {
        const body = JSON.parse(String(init.body));
        bodies.push(body);
        managed = { ...managed, ...body, version: managed.version + 1 };
        return json(managed);
      }
  ...
  it("enforces compatible memberships and clears them when the role changes", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Create user" }));
    expect(
      await screen.findByText("Select at least one compatible organisation unit."),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("This role does not require an organisation membership."),
    ).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Create user" }));
    expect(submit).toHaveBeenCalledWith(
      expect.objectContaining({ organisationUnitIds: [] }),
      expect.anything(),
    );
  });
  ...
  it("reports editor failures and prevents the current Administrator deactivating themselves", async () => {
    ...
    mockFetch((url, init) => {
      if (url.pathname.endsWith("/auth/me")) return json(adminSession);
      if (url.pathname.endsWith("/organisation/units")) return json({ items: organisationUnits });
      if (url.pathname.endsWith(`/admin/users/${selfUser.id}`) && init.method === "PATCH")
        return json({ detail: { message: "Version conflict" } }, 409);
      if (url.pathname.endsWith(`/admin/users/${selfUser.id}`))
        return failUser ? json({ detail: "Missing" }, 404) : json(selfUser);
      throw new Error(url.pathname);
    });
    const user = userEvent.setup();
    const view = renderApp(`/admin/users/${selfUser.id}`);
    expect(
      await screen.findByRole("heading", { name: "User profile could not be loaded" }),
    ).toBeInTheDocument();
    failUser = false;
    await user.click(screen.getByRole("button", { name: "Try again" }));
  ...
  it("requires organisation metadata before editing", async () => {
    ...
    mockFetch((url) => {
      ...
      if (url.pathname.endsWith("/organisation/units"))
        return fail ? json({ detail: "Unavailable" }, 503) : json({ items: organisationUnits });
      if (url.pathname.endsWith(`/admin/users/${adminManagedUser.id}`))
        return json(adminManagedUser);
      throw new Error(url.pathname);
    });
    const user = userEvent.setup();
