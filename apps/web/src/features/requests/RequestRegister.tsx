  ...
  return (
    <div aria-label="Requests" className="request-register" role="table">
      <div className="request-register__header" role="row">
        <span role="columnheader">Reference</span><span role="columnheader">Request</span><span role="columnheader">Status</span><span role="columnheader">Current owner</span><span role="columnheader">Required by</span><span role="columnheader">Actions</span>
      </div>
...
function RequestRow({ request }: { request: RequestSummary }) {
  ...
  return (
    <div className="request-row" role="row">
          <span
            aria-hidden="true"
            className="request-row__indicator"
            role="presentation"
          />
          <span className="mono-ref" role="cell">{request.reference}</span>
          <strong role="cell"><Link className="request-row__title" to={`/requests/${request.id}`}>{request.title}</Link></strong>
          <span className="request-row__status" role="cell"><StatusPill status={request.status} /></span>
          ...
          <time className={`required-signal required-signal--${required.tone}`} dateTime={request.requiredBy} role="cell">
            {required.label}
