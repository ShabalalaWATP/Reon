...
export function DraftRegister({ items }: { items: RequestDraft[] }) {
  ...
  return (
    <section className="register-section" aria-labelledby="draft-register-title">
      <header><span>Private work</span><h2 id="draft-register-title">Drafts</h2></header>
      <div className="request-register">
        {items.map((draft) => (
          <Link className="request-row request-row--draft" key={draft.id} to={`/requests/drafts/${draft.id}`}>
