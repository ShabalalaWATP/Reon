...
const managerAccess: TeamWorkspaceAccess = { teamId: "team-ssg", teamCode: "SSG_TEAM", teamName: "SSG Team", unitKind: "TEAM", workspacePosition: "MANAGER", grantId: "grant-ssg", permissions: ["BOARD", "CALENDAR", "CAPACITY", "ROSTER", "STATISTICS"], views: ["OVERVIEW", "BOARD", "CALENDAR", "PEOPLE", "STATISTICS", "ACTIVITY"] };
...
const seededRecords = [
  record({}),
  record({ id: "record-link", kind: "LINK", title: "Team reference library", body: "Synthetic shared reference.", url: "https://example.test/library", version: 1, updatedAt: "2026-08-11T09:00:00Z" }),
  record({ id: "record-closed", status: "RESOLVED", title: "Superseded notice", resolution: "Replaced by the current notice." }),
];

type RecordedCall = { path: string; body: Record<string, unknown> };

function mockOverview(session: Session, access: TeamWorkspaceAccess, calls: RecordedCall[], options: { empty?: boolean; mutationFailure?: boolean; recordsFailure?: boolean } = {}) {
  ...
  return mockFeatureFetch(async (url, init) => {
    ...
    if (url.pathname.endsWith("/board")) return json({ items: [], nextCursor: null, columnCounts: {}, totalCount: 0, wipLimits: {}, configurationVersion: 0, savedViews: [], generatedAt: "2026-08-13T09:00:00Z" });
    if (url.pathname.endsWith("/records") && init.method === "POST") {
      ...
      items = [...items, record({ id: `record-${items.length}`, kind: body.kind as WorkspaceRecord["kind"], title: String(body.title), body: String(body.body), url: (body.url as string | undefined) ?? null, updatedAt: "2026-08-13T10:00:00Z" })];
      return json({ items });
    }
    ...
    if (url.pathname.endsWith("/records")) return options.recordsFailure ? json({ detail: "Unavailable" }, 503) : json({ items });
    if (url.pathname.endsWith(`/team-workspaces/${access.teamId}`)) return json({ access, managerCount: 2, analystCount: 5, activeWorkCount: 1, dueSoonCount: 0, overdueCount: 0 });
...
describe("team noticeboard and pinned links", () => {
  it("lets a Manager read, post, pin and archive shared workspace records", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Noticeboard and pinned links" })).toBeInTheDocument();
    expect(await screen.findByText("Shift handover expectations")).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "Team reference library" })).toHaveAttribute("href", "https://example.test/library");
    ...
    await user.type(screen.getByLabelText(/^Notice detail/), "The synthetic rotation starts on Monday.");
    ...
    expect(calls.at(-1)?.body).toMatchObject({ grantId: "grant-ssg", kind: "HANDOVER", title: "Watchkeeper rotation" });
    ...
    expect(screen.getByRole("button", { name: "Archive Shift handover expectations" })).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Archive Shift handover expectations" }));
    const archiveForm = screen.getByRole("button", { name: "Confirm archive" }).closest("form")!;
    await user.type(within(archiveForm).getByLabelText(/^Reason/), "This notice has been replaced by the rotation notice.");
    await user.click(screen.getByRole("button", { name: "Confirm archive" }));
    await waitFor(() => expect(screen.queryByText("Shift handover expectations")).not.toBeInTheDocument());
