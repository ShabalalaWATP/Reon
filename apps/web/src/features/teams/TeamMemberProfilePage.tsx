...
export function TeamMemberProfilePage() {
  ...
  const profile = useQuery({
    queryKey: queryKeys.teamMemberProfile(teamId, memberId),
      ...
      <PageState
        action={
          <div className="page-state__actions">
            <Link className="button button--quiet" to={`/teams/${teamId}/people`}>
        ...
        kind="error"
        title="Team member profile could not be loaded"
      >
        The person may no longer be visible in this workspace.
      </PageState>
    );
  }
  ...
  return (
    <main className="page-stack profile-page team-member-profile">
      <Link className="team-profile-back" to={`/teams/${member.teamId}/people`}>
        <ArrowLeft aria-hidden="true" size={16} />
        Back to {member.teamName} people
      </Link>
      <header className="profile-identity">
        ...
        <strong
          className={
            member.accountActive ? "profile-status" : "profile-status profile-status--inactive"
          }
        >
          <BadgeCheck aria-hidden="true" size={16} />
          {member.accountActive ? "Active account" : "Inactive account"}
        </strong>
      </header>
        ...
        <section aria-labelledby="member-identity-title">
          <header>
            ...
            <div>
              ...
              <h2 id="member-identity-title">Profile details</h2>
            </div>
          </header>
          <dl className="profile-definition-list">
            ...
            <div>
              <dt>Rank or grade</dt>
              <dd>{member.rankOrGrade ?? "Not provided"}</dd>
            </div>
            <div>
              <dt>Work email</dt>
              <dd>
        ...
        <section aria-labelledby="member-team-title">
          <header>
            ...
            <div>
              ...
              <h2 id="member-team-title">Team membership</h2>
            </div>
          </header>
          <dl className="profile-definition-list">
            <div>
            ...
            <div>
              <dt>Position</dt>
              <dd>{member.workspacePosition === "MANAGER" ? "Manager" : "Member"}</dd>
            </div>
            <div>
              <dt>Membership</dt>
              <dd>{sentenceCase(member.membershipState)}</dd>
            </div>
            <div>
              <dt>Skills</dt>
              <dd>{member.skills.join(", ") || "Not listed"}</dd>
            </div>
          </dl>
        </section>
      </div>
      <p className="profile-note team-member-profile__privacy">
        Private profile notes and service numbers are not shared through team workspaces.
      </p>
    </main>
  );
}
