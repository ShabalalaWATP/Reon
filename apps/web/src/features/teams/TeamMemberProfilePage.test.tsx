...
describe("team member profile", () => {
  it("opens a bounded colleague profile and returns to the People register", async () => {
    mockFeatureFetch(async (url) => {
      ...
      if (url.pathname.endsWith("/people/analyst-ssg/profile")) return json({ accountId: "analyst-ssg", name: "Lewis Ferguson", email: "admin11@istari.example.test", role: "DELIVERY_SPECIALIST", teamId: access.teamId, teamName: access.teamName, workspacePosition: "MEMBER", membershipState: "CURRENT", rankOrGrade: "Analyst", skills: member.skills, accountActive: true });
    ...
    expect(await screen.findByRole("table", { name: "Workspace membership history" })).toBeInTheDocument();
  ...
  it("recovers a failed profile read and renders explicit empty and inactive states", async () => {
    ...
    mockFeatureFetch(async (url) => {
      ...
      if (url.pathname.endsWith("/people/analyst-ssg/profile")) {
        ...
        return json({ accountId: "analyst-ssg", name: "Lewis Ferguson", email: "admin11@istari.example.test", role: "DELIVERY_SPECIALIST", teamId: access.teamId, teamName: access.teamName, workspacePosition: "MANAGER", membershipState: "ENDED", rankOrGrade: null, skills: [], accountActive: false });
      }
      throw new Error(`Unexpected ${url.pathname}`);
    }, true, true, false);
    const user = userEvent.setup();
