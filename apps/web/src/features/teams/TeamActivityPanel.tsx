...
export function TeamActivityPanel({ teamId }: { teamId: string; userId: string }) {
  ...
  if (query.isError)
    return (
      <PageState
        ...
        kind="error"
        title="Team activity could not be loaded"
      />
    );
  if (query.data.items.length === 0)
    return (
      <PageState kind="empty" title="No team activity recorded">
        Roster and planning changes will appear here as immutable metadata events.
      </PageState>
    );
  return (
      ...
      <ol className="team-activity-list">
        {query.data.items.map((event) => (
          <li key={event.id}>
            ...
            <div>
              <strong>{event.subjectDisplayName}</strong>
              <p>{event.summary}</p>
              <small>
                {event.actorDisplayName
                  ? `Recorded by ${event.actorDisplayName}`
                  : "Applied by the scheduled membership service"}
              </small>
            </div>
          </li>
        ))}
