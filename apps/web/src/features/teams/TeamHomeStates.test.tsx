...
describe("team home operational states", () => {
  ...
  it("states clearly when a delivery team has no recent activity", async () => {
    mockFetch((url) => {
      if (url.pathname.endsWith("/board")) return json({ items: [], nextCursor: null, columnCounts: {}, totalCount: 0, wipLimits: {}, configurationVersion: 1, savedViews: [], generatedAt: "2026-08-10T09:00:00Z" });
      if (url.pathname.endsWith("/people") || url.pathname.endsWith("/calendar") || url.pathname.endsWith("/activity")) return json({ items: [] });
      throw new Error(url.pathname);
    });
    render(providers(<DeliveryTeamHome access={deliveryAccess} overview={overview(deliveryAccess)} userId="manager-one" />));
  ...
  it("presents populated routing stages and shared unit context", async () => {
    ...
    render(providers(<RoutingTeamHome access={routingAccess} overview={overview(routingAccess)} userId="manager-one" />));
