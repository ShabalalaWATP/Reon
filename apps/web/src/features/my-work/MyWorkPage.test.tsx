...
describe("My actions", () => {
  it("presents role work, saved views, safe links and configurable columns accessibly", async () => {
    ...
    mockFetch((url, init) => {
      ...
      if (url.pathname.endsWith("/saved-views") && init.method === "POST") return json({ ...workspace.savedViews[0], id: "created" });
      if (url.pathname.includes("/saved-views/") && init.method === "PATCH") return json({ ...workspace.savedViews[0], version: 5 });
      if (url.pathname.includes("/saved-views/") && init.method === "DELETE") return new Response(null, { status: 204 });
    ...
    await waitFor(() => expect(calls.some(({ path }) => path.includes("actionTypes=QUALITY_REVIEW") && path.includes("dueBefore=2026-08-12"))).toBe(true));
  });
  ...
  it("loads another page and supports section and column controls", async () => {
    ...
    mockFetch((url) => {
      ...
      if (url.pathname.endsWith("/me/actions")) {
        if (url.searchParams.has("cursor")) return nextPageReady.then(() => json({ ...workspace, items: [{ ...workspace.items[0], id: "action-4", reference: "ISR-104" }], nextCursor: null }));
        const response = { ...workspace, items: [workspace.items[0]], nextCursor: first ? "page-2" : null, freshness: { ...workspace.freshness, status: "STALE" as const, pendingCount: 0 } };
        first = false;
  ...
  it("shows empty, denied, conflict and retryable states", async () => {
    ...
    mockFetch((url, init) => {
      if (url.pathname.endsWith("/auth/me")) return json(staffSession);
      if (url.pathname.endsWith("/me/capabilities")) return json(enabledCapabilities);
      if (url.pathname.endsWith("/saved-views") && init.method === "POST") return json({ detail: { message: "Saved view changed" } }, 409);
      if (url.pathname.endsWith("/me/actions")) {
    ...
    expect(await screen.findByRole("heading", { name: "Your access has changed" })).toBeInTheDocument();
    denied.unmount();
    response = 409;
    const conflict = renderApp("/my-work");
    expect(await screen.findByRole("heading", { name: "This work view changed" })).toBeInTheDocument();
    conflict.unmount();
    response = 500;
    renderApp("/my-work");
    expect(await screen.findByRole("heading", { name: "Your work could not be loaded" })).toBeInTheDocument();
