...
export function MyWorkPage() {
  ...
  return <main className="page-stack my-work-page">
    ...
    <div className="work-counts" aria-label="Work counts" role="group">{actionSections.map((section) => <button aria-label={`${sectionLabels[section]} ${first.counts[sectionCountKeys[section]]}`} className={filters.sections[0] === section ? "work-count work-count--active" : "work-count"} key={section} onClick={() => setFilters((current) => ({ ...current, sections: current.sections[0] === section ? [] : [section] }))} type="button"><span>{sectionLabels[section]}</span><strong>{first.counts[sectionCountKeys[section]]}</strong></button>)}</div>
    ...
    <section className="work-tools" aria-label="Work view controls">
      ...
      <details className="work-filter-disclosure">
        <summary>Filters and visible columns</summary>
