...
export function NotificationRegister({ items, selected, toggle }: Props) {
  return (
    <ol className="notification-register" aria-label="Notifications">
      {items.map((item) => {
        const href = safeWorkspaceHref(item.deepLink);
        return (
          <li
            className={
              item.isRead ? "notification-row" : "notification-row notification-row--unread"
            }
            key={item.id}
          >
            ...
            <div className="notification-main">
              <span className="mono-ref">{humaniseCode(item.eventGroup)}</span>
              <strong>{item.subject}</strong>
              <small>
                {humaniseCode(item.eventType)} · {formatActionDate(item.occurredAt)}
              </small>
            </div>
            <div className="notification-state">
              {item.isArchived ? (
                ...
                <span className="status-pill status-pill--success">Action complete</span>
              ) : item.isRead ? (
                <span className="status-pill">Read</span>
              ) : (
                <span className="status-pill status-pill--active">Unread</span>
              )}
            </div>
            <div>
              {href ? (
                ...
                <span className="work-link-ended">Access ended</span>
              ) : null}
            </div>
          </li>
        );
