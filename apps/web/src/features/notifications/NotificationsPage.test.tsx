...
import type { NotificationList, NotificationPreferences } from "../../lib/api/actionNotificationTypes";
...
const notifications: NotificationList = {
  ...
  unreadCount: 1,
  nextCursor: null,
  freshness: { status: "DEGRADED", projectedAt: null, sourceChangedAt: null, lagSeconds: null, pendingCount: 0 },
};
...
describe("Notifications", () => {
  it("shows safe notification summaries, bulk actions and preferences accessibly", async () => {
    ...
    mockFetch((url, init) => {
      ...
      if (url.pathname.endsWith("/me/notifications/preferences") && !init.method) return json(preferences);
      if (url.pathname.includes("/me/notifications/preferences/") && init.method === "PATCH") return failPreference ? json({ detail: { message: "Preference changed" } }, 409) : json(preferences.groups[1]);
      if (url.pathname.endsWith("/me/notifications/state")) return json({ items: [] });
      if (url.pathname.endsWith("/me/notifications/count")) return json({ unreadCount: 1, projectedAt: null });
    ...
    expect(JSON.parse(stateCall.init.body as string)).toEqual({ action: "MARK_READ", targets: [{ id: "notice-1", expectedVersion: 2 }] });

    await user.click(screen.getByLabelText("Select Product ready"));
    await user.click(screen.getByRole("button", { name: "Archive" }));
    await waitFor(() => expect(JSON.parse(calls.filter(({ path }) => path === "/api/v1/me/notifications/state").at(-1)!.init.body as string).action).toBe("ARCHIVE"));
    ...
    await waitFor(() => expect(calls.some(({ path }) => path.endsWith("/preferences/WORK"))).toBe(true));
    expect(JSON.parse(calls.find(({ path }) => path.endsWith("/preferences/WORK"))!.init.body as string)).toMatchObject({ enabled: false, reminderDays: [3, 2, 1], expectedVersion: 2 });
    await waitFor(() => expect(within(optional).getByRole("button", { name: "Save" })).not.toBeDisabled());
    failPreference = true;
    await user.click(within(optional).getByRole("button", { name: "Save" }));
    ...
    await waitFor(() => expect(calls.some(({ path }) => path.includes("states=UNREAD") && path.includes("from=2026-08-01T00%3A00%3A00.000Z") && path.includes("to=2026-08-07T23%3A59%3A59.999Z"))).toBe(true));
  ...
  it("loads another page", async () => {
    ...
    mockFetch((url) => {
      ...
      if (url.pathname.endsWith("/me/notifications/count")) return json({ unreadCount: 1, projectedAt: null });
      if (url.pathname.endsWith("/me/notifications")) return url.searchParams.has("cursor")
        ? nextPageReady.then(() => json({ ...notifications, items: [{ ...notifications.items[0], id: "notice-later", subject: "Later update" }], nextCursor: null }))
        : json({ ...notifications, items: [notifications.items[0]], nextCursor: "page-2", freshness: { ...notifications.freshness, status: "CURRENT", pendingCount: 1 } });
  ...
  it("shows empty, denied and retryable states", async () => {
    ...
    mockFetch((url) => {
      if (url.pathname.endsWith("/auth/me")) return json(requesterSession);
      if (url.pathname.endsWith("/me/capabilities")) return json(enabledCapabilities);
      if (url.pathname.endsWith("/me/notifications/preferences")) return response === 500 ? json({ detail: "Unavailable" }, 500) : json({ groups: [] });
      if (url.pathname.endsWith("/me/notifications/count")) return json({ unreadCount: 0, projectedAt: null });
      if (url.pathname.endsWith("/me/notifications")) {
        if (response === 403) return json({ detail: "Denied" }, 403);
        return json({ ...notifications, items: [], unreadCount: 0, freshness: { ...notifications.freshness, status: "CURRENT" } });
      }
      throw new Error(`Unexpected ${url.pathname}`);
    }, true, true, true, true, false, false);
    const empty = renderApp("/notifications");
    expect(await screen.findByRole("heading", { name: "No notifications in this view" })).toBeInTheDocument();
    empty.unmount();
    response = 403;
    const denied = renderApp("/notifications");
    expect(await screen.findByRole("heading", { name: "Notification access ended" })).toBeInTheDocument();
    denied.unmount();
    response = 500;
    renderApp("/notifications");
    expect(await screen.findByRole("heading", { name: "Notifications could not be loaded" })).toBeInTheDocument();
