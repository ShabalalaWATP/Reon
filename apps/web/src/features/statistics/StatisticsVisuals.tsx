...
export function CategoryPanel({ rows, title }: { rows: CategoryCount[]; title: string }) {
  ...
  return (
    <section className="statistics-panel">
      ...
      {rows.length === 0 ? (
        <p className="inline-empty">No records in this period.</p>
      ) : (
        <>
          <div className="category-visual">
            ...
            <ul className="donut-legend">
              {rows.map((row, index) => (
                <li key={row.key}>
                  <i aria-hidden="true" style={{ background: sliceColour(index) }} />
                  <span>{row.label}</span>
...
export function ThroughputPanel({
  ...
  return (
    <section className="statistics-panel statistics-panel--wide">
      <header>
        <h2>{resolutionLabel(resolution)} throughput</h2>
        <span>Received and completed</span>
      </header>
      <div aria-hidden="true" className="throughput-chart">
        {rows.map((row) => (
          <div className="throughput-chart__day" key={row.date} title={row.date}>
            <i
              className="throughput-chart__received"
              style={{ height: `${(row.received / maximum) * 100}%` }}
            />
            <i
              className="throughput-chart__completed"
              style={{ height: `${(row.completed / maximum) * 100}%` }}
            />
          </div>
        ))}
      </div>
      <p className="chart-legend">
        <i /> Received <i /> Completed
      </p>
      <DataTable
...
export function DurationPanel({ rows }: { rows: StageDuration[] }) {
  ...
  return (
    <section className="statistics-panel statistics-panel--wide">
      <header>
        <h2>Completed stage duration</h2>
        <span>Median and 90th percentile</span>
      </header>
      {rows.length === 0 ? (
        <p className="inline-empty">No completed stages in this period.</p>
      ) : (
        <>
          <div aria-hidden="true" className="duration-range-chart">
            {rows.map((row) => (
              <div key={row.key}>
                <span>{row.label}</span>
                <i>
                  <b style={{ width: `${(row.p90Hours / maximum) * 100}%` }} />
                  <em style={{ left: `${(row.medianHours / maximum) * 100}%` }} />
                </i>
                <strong>{row.medianHours} h</strong>
                <small>{row.p90Hours} h</small>
              </div>
            ))}
          </div>
          <p className="duration-legend">
            <i /> Median <i /> 90th percentile
          </p>
          <DataTable
...
export function ChildrenPanel({
  ...
  return (
    <section className="statistics-panel statistics-panel--wide">
      <header>
        <h2>Direct child units</h2>
        <span>Demand within this scope</span>
      </header>
      <div className="child-comparison">
        {rows.map((row) => (
          ...
          >
            <span>{row.name}</span>
            <div>
              <i style={{ width: `${(row.received / maximum) * 100}%` }} />
            </div>
