...
import {
  safeStatisticsExportDownloadUrl,
  statisticsEvolutionApi,
} from "../../lib/api/statisticsEvolutionClient";
import type {
  ExportPolicy,
...
export function StatisticsExportPanel({
  ...
  return (
    <section className="statistics-export">
      <header>
        <div>
          <span>Audited aggregate output</span>
          <h3>Controlled export</h3>
        </div>
        <p>
          The server repeats scope, date, cohort and suppression checks before producing a file.
        </p>
      </header>
  ...
  return (
    <div>
      <button
        ...
        onClick={() => mutation.mutate(format)}
        type="button"
      >
        Prepare {format}
      </button>
      <span>
        {available ? "Available for this aggregate scope" : `${policy.state}: ${policy.reason}`}
      </span>
    </div>
  );
}
...
function ExportResult({ result }: { result: StatisticsExportResult }) {
  const downloadUrl = safeStatisticsExportDownloadUrl(result.downloadUrl);
  return (
    <div aria-live="polite" className="statistics-export__result">
      <strong>{result.state === "READY" ? "Export ready" : "Export pending"}</strong>
      <span>{result.message}</span>
      {result.state === "READY" && downloadUrl ? (
        <a className="button button--primary" href={downloadUrl}>
          Download aggregate export
        </a>
      ) : null}
      {result.state === "READY" && result.downloadUrl && !downloadUrl ? (
        <span>Download address rejected by the client safety policy.</span>
      ) : null}
    </div>
  );
}
