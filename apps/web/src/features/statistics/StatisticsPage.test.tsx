...
describe("operational statistics", () => {
  it("renders a neutral donut when a populated category set totals zero", () => {
    const view = render(
      <CategoryPanel
        rows={[{ key: "none", label: "No overdue work", count: 0 }]}
        title="Due-date risk"
      />,
    );
    ...
    expect(screen.getByRole("table", { name: "Due-date risk data" })).toHaveTextContent(
      "No overdue work",
    );
  });
  ...
  it("shows only granted scopes with accessible chart-table parity", async () => {
    ...
    mockFeatureFetch(
      (url) => {
        ...
        if (url.pathname.endsWith("/statistics")) {
          ...
          const selectedScope =
            requestedScopes.at(-1) === commandScope.id ? commandScope : platformScope;
          const selectedUnit =
            selectedScope.units.find((unit) => unit.id === requestedUnits.at(-1)) ??
            selectedScope.units[0];
          return json({
            ...dashboard,
            scope: selectedScope,
            selectedUnit,
            breadcrumb: selectedUnit.parentId
              ? [selectedScope.units[0], selectedUnit]
              : [selectedUnit],
          });
        }
    ...
    expect(
      within(screen.getByRole("region", { name: "Summary measures" })).getByText("8"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("table", { name: "Direct child unit comparison data" }),
    ).toHaveTextContent("SYGOC");
    ...
    expect(
      await screen.findByText("ACSA-B Ops", { selector: ".statistics-filters option" }),
    ).toBeInTheDocument();
  ...
  it("handles unavailable access and retries scope failures", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "No statistics scope assigned" }),
    ).toBeInTheDocument();
  ...
  it("reports dashboard failures and renders an empty degraded projection", async () => {
    ...
    mockFeatureFetch(
      (url) => {
        ...
        if (url.pathname.endsWith("/statistics")) {
          ...
          return json({
            ...dashboard,
            summary: dashboard.summary.map((metric) =>
              metric.key === "received" ? { ...metric, value: null } : metric,
            ),
            freshness: {
              health: "REBUILDING",
              lastProjectedAt: null,
              sourceEventCount: 0,
              projectedRequestCount: 0,
            },
            status: [],
            age: [],
            dueRisk: [],
    ...
    expect(
      screen.queryByRole("table", { name: "Direct child unit comparison data" }),
