...
export function StatisticsEvolutionView({
  ...
  const suppressedCount =
    data.comparison.filter((row) => row.suppressed).length +
  ...
  const detailCount =
    Object.values(visible).reduce((total, rows) => total + rows.length, 0) +
  ...
  return (
    <section className="statistics-evolution">
      <header className="statistics-evolution__heading">
        <div>
          <span>Scoped operational evidence</span>
          <h2>Planning and service flow</h2>
          <p>
            Aggregate comparisons use the selected grant and contain no request content or
            individual ranking.
          </p>
        </div>
        <small>{context}</small>
      ...
      {suppressedCount > 0 ? (
        <p className="statistics-privacy-note">
          {suppressedCount} detailed measure{suppressedCount === 1 ? " is" : "s are"} hidden because
          the selected cohort is too small.
        </p>
      ) : null}
      {detailCount > 0 ? (
        <div className="statistics-evolution-grid">
          {visible.comparison.length ? (
            <ComparisonPanel context={context} rows={visible.comparison} />
          ) : null}
          {visible.bottlenecks.length ? (
            <BottleneckPanel context={context} rows={visible.bottlenecks} />
          ) : null}
          {data.capacity.length ? (
            <CapacityTrendPanel context={context} rows={data.capacity} />
          ) : null}
          {visible.releases.length ? (
            <ReleasePanel context={context} rows={visible.releases} />
          ) : null}
          {visible.notifications.length ? (
            <NotificationPanel context={context} rows={visible.notifications} />
          ) : null}
          {visible.iterations.length ? (
            <IterationPanel context={context} rows={visible.iterations} />
          ) : null}
          {data.projection.periods.length ? (
        ...
        <p className="inline-empty">
          No detailed operational measures are available for this scope and period.
        </p>
      )}
      <StatisticsExportPanel filters={filters} policies={data.exports} session={session} />
    </section>
  );
}
