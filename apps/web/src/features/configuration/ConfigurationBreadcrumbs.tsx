...
export function ConfigurationBreadcrumbs({
  ...
  if (!path.length)
    return (
      <p className="configuration-breadcrumb-empty">
        Select a unit to see its organisational path.
      </p>
    );
  return (
    <nav aria-label="Selected organisation path" className="configuration-breadcrumbs">
      <ol>
        {path.map((unit, index) => (
          <li key={unit.unitId}>
            {index ? <ChevronRight aria-hidden="true" size={14} /> : null}
            <button
              aria-current={unit.unitId === selectedId ? "location" : undefined}
