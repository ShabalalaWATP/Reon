...
export function ConfigurationTree({
  ...
  const rows = useMemo(
    () => configurationRows(units, edges, effectiveAt),
    [edges, effectiveAt, units],
  );
  const visibleRows = useMemo(() => filterConfigurationRows(rows, search), [rows, search]);
  if (!rows.length)
    return <p className="inline-empty">No organisation units have been configured.</p>;
  ...
  return (
    <div className="configuration-tree-panel">
      <label className="configuration-tree-search">
        ...
        <span>
          ...
          <input
            ...
            placeholder="Name, code or unit type"
            type="search"
            value={search}
          />
          {search ? (
      ...
      <p aria-live="polite" className="configuration-tree-results">
        {search.trim()
          ? `${visibleRows.filter((row) => matchesConfigurationRow(row, search)).length} matching units, with organisational context`
          : `${rows.length} organisation units`}
      </p>
      {!visibleRows.length ? (
        <p className="inline-empty" role="status">
          No organisation units match “{search.trim()}”.
        </p>
      ) : (
        <div aria-label="Organisation hierarchy" className="configuration-tree" role="tree">
          {visibleRows.map((unit, index) => (
            <button
              ...
              className={
                selectedId === unit.unitId
                  ? "configuration-tree__item configuration-tree__item--selected"
                  : "configuration-tree__item"
              }
              ...
              onKeyDown={(event) => {
                if (event.key === "ArrowDown") {
                  ...
                  moveFocus(Math.min(index + 1, visibleRows.length - 1));
                }
            ...
            >
              ...
              <span>
                ...
                <small>
                  {unit.code} · {unit.kind.replace("_", " ").toLowerCase()}
                </small>
              </span>
