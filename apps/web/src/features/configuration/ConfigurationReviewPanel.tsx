  ...
  return (
    <aside className="configuration-review" aria-labelledby="configuration-review-title">
      ...
      <section aria-labelledby="change-preview-title" className="configuration-review__section">
        ...
        {preview?.changes.length ? <ol className="configuration-change-list">{preview.changes.map((change, index) => <li key={`${change.unitId}-${change.type}-${index}`}><span>{change.type.replace("_", " ")}</span><strong>{change.code}</strong><p>{change.message}</p><button className="text-button" onClick={() => onFocusUnit(change.unitId)} type="button">Show in tree</button></li>)}</ol> : null}
      </section>
      ...
      {!independent && version.status === "AWAITING_APPROVAL" ? <p className="form-banner form-banner--warning" role="status">A different Platform Administrator must approve this version.</p> : null}
...
function actionLabel(action: Action) {
  const labels: Record<Action, string> = { activate: "Activate approved version", approve: "Approve exact version", reject: "Reject version", submit: "Submit for independent approval", validate: "Validate complete configuration" };
  return labels[action];
}

function ConfigurationHistory({ version }: { version: ConfigurationVersion }) {
  const events = [
    { at: version.createdAt, label: "Draft created" },
