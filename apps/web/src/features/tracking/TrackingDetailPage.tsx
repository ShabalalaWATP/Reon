...
export function TrackingDetailPage() {
  ...
  const query = useQuery({
    queryKey: queryKeys.trackedRequest(requestId),
  ...
  if (query.isError)
    return (
      <PageState
        action={
          <Link className="button" to="/tracking">
            Back to tracking
          </Link>
        }
        kind="error"
        title="Tracked request not available"
      >
        It may no longer exist or sit within your authorised route.
      </PageState>
    );
  const request = query.data;
  return (
    <main className="page-stack tracking-detail">
      <Link className="back-link" to="/tracking">
      ...
      <header className="detail-heading">
        <div>
          ...
          <h1>{request.title}</h1>
          <p>
            Submitted by {request.requesterDisplayName} on {formatDate(request.createdAt)}.
          </p>
        </div>
        <StatusPill status={request.status} />
      </header>
      <div className="tracking-read-only">
        <strong>Read-only lifecycle view</strong>
        <span>Return to the role queue to take an action.</span>
      </div>
      ...
      <TrackingActivity
        ...
        key={`${request.updatedAt}-${request.events.length}`}
        requestId={request.id}
      />
    </main>
  );
  ...
  return (
    <section aria-labelledby="tracking-overview-title" className="detail-section">
      <div className="section-heading">
        ...
        <h2 id="tracking-overview-title">Request details</h2>
      </div>
