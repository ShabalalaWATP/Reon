    ...
    <details className="tracking-filters">
      ...
      >
        ...
        <label className="form-field">
          ...
          <select
            onChange={(event) =>
              onChange({ ...draft, status: event.target.value as TrackedRequestFilters["status"] })
            }
            value={draft.status}
          >
        ...
        <label className="form-field">
          ...
          <input
            maxLength={120}
            onChange={(event) => onChange({ ...draft, currentOwner: event.target.value })}
        ...
        <label className="form-field">
          ...
          <select
            onChange={(event) => onChange({ ...draft, routeUnitId: event.target.value })}
            value={draft.routeUnitId}
          >
        ...
        <label className="form-field">
          <span>Open for at least</span>
          <select
            onChange={(event) => onChange({ ...draft, minimumAgeDays: event.target.value })}
        ...
        <div className="tracking-filter-actions">
          ...
          <button
            ...
            disabled={!active && !Object.values(draft).some(Boolean)}
            onClick={onClear}
            type="button"
          >
            Clear
