...
export function TrackingJourney({ request, viewer }: { request: TrackedRequest; viewer?: JourneyViewer }) {
    ...
    <section aria-label={`Request lifecycle for ${request.reference}`} className="tracking-journey">
      ...
      <div className="tracking-journey__route">
        <div className="tracking-journey__label">
          <strong>Selected route</strong><span>Where the request has travelled</span>
          {viewer ? <em>Viewing as {viewer.displayName}</em> : null}
        </div>
        <ol>
          <li className="tracking-route-origin" data-state="complete"><i aria-hidden="true" /><strong>Customer</strong><small>Submitted request</small></li>
          {request.route.length ? request.route.map((unit, index) => {
            ...
            return (
              <li data-state={state} data-viewer={viewerUnit || undefined} key={unit.id}>
                <i aria-hidden="true" /><strong>{unit.name}</strong><small>{stateLabel}</small>
                {viewerUnit ? <em>Your unit</em> : null}
              </li>
            );
          }) : <li className="tracking-route-pending"><strong>Routing decision</strong><small>Waiting for the first routing decision.</small></li>}
          <li aria-current="location" className="tracking-route-now">
            <i aria-hidden="true" /><strong>{request.currentOwner ?? "Awaiting routing"}</strong><small>Now · Current owner</small>
          </li>
        </ol>
      </div>
