...
const routeViewerSession = {
  ...staffSession,
  user: { ...staffSession.user, organisationUnitIds: [trackedRequest.route[0].id] },
};
...
describe("route-scoped request tracking", () => {
  it("shows titles, lifecycle graphics and links without loading request content", async () => {
    ...
    const routedRow = screen
      ...
      .closest("article")!;
    expect(
      within(routedRow).getByText("The destination and ownership are being agreed."),
    ).toBeInTheDocument();
    expect(
      within(routedRow)
        .getAllByText("Routing")
    ...
    expect(within(routedRow).getByText("Your unit").closest("li")).toHaveAttribute(
      "data-viewer",
      "true",
    );
    expect(within(routedRow).getByText("Now · Current owner").closest("li")).toHaveAttribute(
      "aria-current",
      "location",
    );
    ...
    expect(
      screen.queryByRole("button", { name: /Approve|Disseminate|Record outcome/ }),
    ).not.toBeInTheDocument();
    expect(paths[0]).toBe("/api/v1/auth/me");
    expect(new Set(paths.slice(1))).toEqual(
      new Set(["/api/v1/organisation/units", "/api/v1/tracked-requests"]),
    );
  ...
  it("opens an authorised historical request as a read-only lifecycle view", async () => {
    ...
    expect(
      screen.getByLabelText(`Request lifecycle for ${trackedRequest.reference}`),
    ).toBeInTheDocument();
    expect(screen.getByText("Request routed to ACSA-B Ops")).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /Record outcome|Disseminate|Assign/ }),
    ).not.toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "Service product" })).not.toBeInTheDocument();
    expect(await axe(view.container)).toHaveNoViolations();
  });
  it("filters monitored work independently from the action queue", async () => {
    ...
    await user.selectOptions(
      screen.getByLabelText("Route destination"),
      organisationUnit("ACSA_B_OPS").id,
    );
            ...
            events: [
              {
                id: "old",
                type: "ROUTED",
                message: "Earlier routing decision",
  ...
  it("reports an unavailable older history page without losing the request", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: "Load more" }));
    expect(
      await screen.findByText("Older ticket history could not be loaded."),
    ).toBeInTheDocument();
  ...
  it("recovers from an error and renders an empty tracker", async () => {
    ...
    renderApp("/tracking");
    expect(
      await screen.findByRole("heading", { name: "Tracking could not be loaded" }),
    ).toBeInTheDocument();
    fail = false;
    await user.click(screen.getByRole("button", { name: "Try again" }));
    expect(
      await screen.findByRole("heading", { name: "No requests to track" }),
    ).toBeInTheDocument();
  });
});
