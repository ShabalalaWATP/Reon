...
export function CapacityPanel({ access }: { access: TeamWorkspaceAccess }) {
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      return api.previewTeamCapacity(
        access.teamId,
        { grantId: access.grantId, dateFrom, dateTo, timeZone },
        session.csrfToken,
      );
    },
  ...
  const commitMutation = useMutation({
    mutationFn: () => {
      if (!session || !access.grantId || !preview)
        throw new Error("Create a current preview first.");
      return api.commitTeamCapacity(
        access.teamId,
        { grantId: access.grantId, token: preview.token },
        session.csrfToken,
      );
    },
  ...
  return (
    <section className="capacity-panel">
      <header>
        <span>Versioned availability</span>
        <h2>Capacity snapshot</h2>
        <p>
          Preview calendar-backed working minutes, then commit only if the source versions are
          unchanged.
        </p>
      </header>
      <form
      ...
      >
        <label className="form-field">
          From<span className="field-hint">Required</span>
          <input
        ...
        <label className="form-field">
          To<span className="field-hint">Required</span>
          <input
            min={dateFrom}
        ...
        <button className="button" disabled={previewMutation.isPending} type="submit">
          Preview capacity
        </button>
      </form>
      {preview ? (
        <>
          <div className="team-table-wrap">
            <table className="team-table">
              <caption>Calendar-backed capacity preview</caption>
              <thead>
                <tr>
              ...
              <tbody>
                {preview.days.map((day) => (
                  <tr key={day.date}>
                    ...
                    <td>{minutes(day.baselineMinutes)}</td>
                    <td>{minutes(day.unavailableMinutes)}</td>
                    <td>{minutes(day.availableMinutes)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          {commitMutation.isError ? <p role="alert">{message(commitMutation.error)}</p> : null}
          <button
...
function message(error: Error) {
  return error instanceof ApiError
    ? error.message
    : error.message || "Capacity could not be calculated.";
}
