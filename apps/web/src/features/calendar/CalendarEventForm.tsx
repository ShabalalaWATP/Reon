...
export function CalendarEventForm({ access, initialDate, members, onCreated, range, sharingUnitName }: {
  ...
  return (
    <section className="calendar-form-panel">
      ...
      <form onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }}>
        ...
        <div className={`calendar-form-grid${mode === "personal" ? "" : " calendar-form-grid--single"}`}>
          <Select label="Category" onChange={(value) => setCategory(value as CalendarCategory)} options={["AVAILABILITY", "SERVICE_WORK", "LEAVE", "TRAINING", "DUTY", "APPOINTMENT", "OTHER"]} value={category} />
          {mode === "personal" ? (
            <label className="calendar-private-choice">
              <input
                checked={visibility === "PRIVATE"}
                onChange={(event) => setVisibility(event.target.checked ? "PRIVATE" : "TEAM_DETAIL")}
                type="checkbox"
              />
              <span>
