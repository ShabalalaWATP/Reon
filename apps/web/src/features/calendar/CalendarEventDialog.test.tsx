...
const calendarSession = { ...requesterSession, user: { ...requesterSession.user, id: "jioc-member", username: "admin75", displayName: "Willie Ormond", role: "INTAKE_TRIAGE" as const, scope: "JIOC" } };
const calendarAccess = { teamId: "jioc", teamCode: "JIOC", teamName: "JIOC", unitKind: "ROOT" as const, workspacePosition: "MEMBER" as const, grantId: null, permissions: [], views: ["OVERVIEW", "QUEUE", "CALENDAR", "PEOPLE", "STATISTICS", "HANDOVER", "ACTIVITY"] };
...
describe("calendar event dialog", () => {
  it("opens one form from the toolbar or a calendar day, pre-fills the day and closes after creation", async () => {
    ...
    mockFeatureFetch(async (url, init) => {
      ...
      if (url.pathname.endsWith("/calendar/events") && init.method === "POST") { writes.push(String(init.body)); return json({ eventId: "event-new", version: 1 }); }
      throw new Error(`Unexpected ${url.pathname}`);
    }, true, true, false);
    const user = userEvent.setup();
    const view = renderApp("/teams/jioc/calendar");
