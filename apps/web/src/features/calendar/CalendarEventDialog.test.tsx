  ...
  it("opens one form from the toolbar or a calendar day, pre-fills the day and closes after creation", async () => {
    ...
    mockFeatureFetch(
      async (url, init) => {
        ...
        if (url.pathname.endsWith("/calendar") && (!init.method || init.method === "GET"))
          return json({ items: [] });
        if (url.pathname.endsWith("/calendar/events") && init.method === "POST") {
          writes.push(JSON.parse(String(init.body)) as Record<string, unknown>);
          return json({ eventId: "event-new", version: 1 });
        }
    ...
    await user.type(
      ...
      "A synthetic calendar event created from the selected day.",
    );
    ...
    await user.type(
      screen.getByLabelText(/^Notes/),
      "Synthetic personal detail hidden from the team calendar.",
    );
    await user.click(screen.getByRole("button", { name: "Create event" }));
