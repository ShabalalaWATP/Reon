...
describe("configuration API client", () => {
  it("uses the immutable registry, preview, snapshot and action contracts", async () => {
    ...
    await configurationApi.submit(
      "cfg/2",
      { expectedVersion: 2, reason: "Ready for review" },
      "csrf",
    );
    await configurationApi.approve(
      "cfg/2",
      { expectedVersion: 3, reason: "Independent approval" },
      "csrf",
    );
    await configurationApi.reject(
      "cfg/2",
      { expectedVersion: 3, reason: "Hierarchy is incomplete" },
      "csrf",
    );
    await configurationApi.activate(
      "cfg/2",
      { expectedVersion: 4, reason: "Activate for future requests" },
      "csrf",
    );
