...
export const requestApi = {
  ...
  updateDraft: (id: string, input: RequestDraftUpdateInput, csrfToken: string) =>
    apiRequest<RequestDraft>(`/request-drafts/${encodeURIComponent(id)}`, { body: input, csrfToken, method: "PATCH" }),
  ...
  submitDraft: (id: string, input: RequestDraftSubmitInput, csrfToken: string) =>
    apiRequest<RequestDetail>(`/request-drafts/${encodeURIComponent(id)}/submit`, { body: input, csrfToken, method: "POST" }),
  request: (id: string, eventCursor?: string) =>
    apiRequest<RequestDetail>(
      pagedPath(`/requests/${encodeURIComponent(id)}`, undefined, eventCursor ? { eventCursor } : {}),
    ),
