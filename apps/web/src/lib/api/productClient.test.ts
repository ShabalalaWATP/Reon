...
describe("product API client", () => {
  it("uses the bounded package and release route contracts", async () => {
    ...
    await productApi.addManagedArtefact(
      "package/one",
      {
        ...versioned,
        filename: "brief.pdf",
    ...
    await productApi.addExternalLink(
      "package/one",
      { ...versioned, label: "Product", url: "https://example.test/a" },
      csrf,
    );
    await productApi.submit("package/one", { ...versioned, coveringNote: "Synthetic note." }, csrf);
    await productApi.managerApprove(
      "package/one",
      { ...versioned, packageChecksum: "b".repeat(64) },
      csrf,
    );
    await productApi.disseminate(
      "package/one",
      { ...versioned, packageChecksum: "b".repeat(64), externalLinkAttested: true },
      csrf,
    );
    ...
    expect(productArtefactUrl("id/one", "MANAGED_FILE")).toBe(
      "/api/v1/releases/artefacts/id%2Fone/download",
    );
    expect(productArtefactUrl("id/one", "EXTERNAL_LINK")).toBe(
      "/api/v1/releases/artefacts/id%2Fone/open",
    );
  });
  ...
  it("uploads raw bytes with the optimistic version, token and CSRF proof", async () => {
    ...
    const receipt = await productApi.uploadContent(
      "package",
      "intent",
      new File(["bytes"], "brief.pdf", { type: "application/pdf" }),
      2,
      "opaque-token",
      csrf,
    );
    ...
    expect(seen?.path).toBe(
      "/api/v1/product-packages/package/uploads/intent/content?expectedVersion=2",
    );
  ...
  it("reports JSON and non-JSON upload failures without exposing token data", async () => {
    ...
    await expect(productApi.uploadContent("p", "i", file, 1, "secret", csrf)).rejects.toEqual(
      expect.objectContaining({ message: "Upload intent expired.", status: 409 }),
    );
    await expect(productApi.uploadContent("p", "i", file, 1, "secret", csrf)).rejects.toEqual(
      expect.objectContaining({ message: "Checksum mismatch.", status: 422 }),
    );
    await expect(productApi.uploadContent("p", "i", file, 1, "secret", csrf)).rejects.toEqual(
      expect.objectContaining({ message: "Upload failed with status 503.", status: 503 }),
    );
