export type CalendarCategory = "AVAILABILITY" | "SERVICE_WORK" | "LEAVE" | "TRAINING" | "DUTY" | "APPOINTMENT" | "OTHER";
...
export interface FutureSplitInput extends CalendarEventInput { expectedVersion: number; splitFrom: string; reason: string }
export interface CommitmentDecisionInput { expectedVersion: number; reason: string | null }
export interface CapacityPreviewInput { grantId: string; dateFrom: string; dateTo: string; timeZone: string }
export interface CapacityCommitInput { grantId: string; token: string }
export interface CapacityDay { date: string; memberCount: number; baselineMinutes: number; unavailableMinutes: number; availableMinutes: number }
export interface CapacityPreview { token: string; expiresAt: string; days: CapacityDay[] }
export interface CapacitySnapshot { snapshotId: string; days: CapacityDay[] }
