...
describe("action and notification API client", () => {
  it("encodes action filters and protects saved-view mutations", async () => {
    ...
    await actionNotificationApi.updateActionView(
      "view/one",
    ...
    expect(calls[0].path).toBe(
      "/api/v1/me/actions?sections=NEEDS_MY_ACTION&actionTypes=LEAD_REVIEW&dueBefore=2026-08-12&limit=20&cursor=next+page",
    );
    expect(
      calls
        .slice(1)
  ...
  it("encodes notification filters, state targets and preferences", async () => {
    ...
    await actionNotificationApi.notifications({
      states: ["UNREAD", "READ"],
      eventTypes: ["ASSIGNED"],
      fromDate: "2026-08-01",
      toDate: "2026-08-07",
    ...
    expect(JSON.parse(calls[2].init.body as string)).toEqual({
      action: "MARK_READ",
      targets: [{ id: "notice-1", expectedVersion: 4 }],
    });
