...
export const calendarApi = {
  ...
  teamCalendar: (teamId: string, from: string, to: string) => {
    const query = new URLSearchParams({ from, to });
    return apiRequest<{ items: CalendarOccurrence[] }>(
      `/team-workspaces/${encodeURIComponent(teamId)}/calendar?${query.toString()}`,
    );
  },
  ...
  createTeamCalendarEvent: (teamId: string, input: TeamCalendarEventInput, csrfToken: string) =>
    apiRequest<CalendarEventResult>(
      `/team-workspaces/${encodeURIComponent(teamId)}/calendar/events`,
      { body: input, csrfToken, method: "POST" },
    ),
  createCalendarCommitment: (teamId: string, input: CommitmentInput, csrfToken: string) =>
    apiRequest<CalendarEventResult>(
      `/team-workspaces/${encodeURIComponent(teamId)}/calendar/commitments`,
      { body: input, csrfToken, method: "POST" },
    ),
  updateCalendarEvent: (eventId: string, input: CalendarEventUpdateInput, csrfToken: string) =>
    apiRequest<CalendarEventResult>(`/calendar/events/${encodeURIComponent(eventId)}`, {
      body: input,
      csrfToken,
      method: "PUT",
    }),
  cancelCalendarEvent: (eventId: string, input: OccurrenceCancelInput, csrfToken: string) =>
    apiRequest<CalendarEventResult>(`/calendar/events/${encodeURIComponent(eventId)}/cancel`, {
      body: input,
      csrfToken,
      method: "POST",
    }),
  cancelCalendarOccurrence: (eventId: string, input: OccurrenceCancelInput, csrfToken: string) =>
    apiRequest<CalendarEventResult>(
      `/calendar/events/${encodeURIComponent(eventId)}/occurrences/cancel`,
      { body: input, csrfToken, method: "POST" },
    ),
  editCalendarOccurrence: (eventId: string, input: OccurrenceEditInput, csrfToken: string) =>
    apiRequest<CalendarEventResult>(
      `/calendar/events/${encodeURIComponent(eventId)}/occurrences/edit`,
      { body: input, csrfToken, method: "POST" },
    ),
  splitCalendarSeries: (eventId: string, input: FutureSplitInput, csrfToken: string) =>
    apiRequest<CalendarEventResult>(`/calendar/events/${encodeURIComponent(eventId)}/split`, {
      body: input,
      csrfToken,
      method: "POST",
    }),
  decideCalendarCommitment: (
    eventId: string,
    ...
    apiRequest<CalendarEventResult>(
      `/calendar/events/${encodeURIComponent(eventId)}/${acknowledge ? "acknowledge" : "dispute"}`,
      { body: input, csrfToken, method: "POST" },
    ),
  previewTeamCapacity: (teamId: string, input: CapacityPreviewInput, csrfToken: string) =>
    apiRequest<CapacityPreview>(
      `/team-workspaces/${encodeURIComponent(teamId)}/capacity/previews`,
      { body: input, csrfToken, method: "POST" },
    ),
  commitTeamCapacity: (teamId: string, input: CapacityCommitInput, csrfToken: string) =>
    apiRequest<CapacitySnapshot>(
      `/team-workspaces/${encodeURIComponent(teamId)}/capacity/commits`,
      { body: input, csrfToken, method: "POST" },
    ),
};
