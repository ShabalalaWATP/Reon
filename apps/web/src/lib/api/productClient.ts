...
const packagePath = (packageId: string) => `${packagesRoot}/${encodeURIComponent(packageId)}`;
...
export const productApi = {
  ...
  packageForRequest: (requestId: string) =>
    apiRequest<ProductPackage | null>(
      `${packagesRoot}/by-request/${encodeURIComponent(requestId)}`,
    ),
  addManagedArtefact: (packageId: string, input: ManagedArtefactInput, csrfToken: string) =>
  ...
  completeUpload: (
    ...
    apiRequest<ProductPackage>(
      `${packagePath(packageId)}/uploads/${encodeURIComponent(intentId)}/complete`,
      {
        body: input,
        csrfToken,
        method: "POST",
  ...
  addExternalLink: (packageId: string, input: ExternalLinkInput, csrfToken: string) =>
  ...
  submit: (
    packageId: string,
    input: VersionedCommand & { coveringNote: string },
    csrfToken: string,
  ...
  withdraw: (packageId: string, input: VersionedCommand & { reason: string }, csrfToken: string) =>
...
export const productArtefactUrl = (
  artefactId: string,
  kind: ProductRelease["artefacts"][number]["kind"],
) =>
