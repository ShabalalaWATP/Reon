...
describe("API client", () => {
  it("calls every endpoint with cookies, JSON and CSRF protection", async () => {
    ...
    mockFetch((url, init) => {
      ...
      if (url.pathname.endsWith("/auth/login") || url.pathname.endsWith("/auth/me")) return json(requesterSession);
      ...
      if (url.pathname.includes("/admin/users")) return url.pathname.endsWith("/admin/users") ? json({ items: [adminManagedUser] }) : json(adminManagedUser);
      if (url.pathname.endsWith("/feedback")) return json({ id: "feedback", rating: 5, comments: "Very useful.", createdAt: requestDetail.updatedAt });
      if (url.pathname.endsWith("/request-drafts/draft-id/submit")) return json(requestDetail);
      if (url.pathname.endsWith("/request-drafts/draft-id") && init.method === "DELETE") return new Response(null, { status: 204 });
      if (url.pathname.endsWith("/request-drafts/draft-id")) return json(draft);
      if (url.pathname.endsWith("/request-drafts")) return init.method === "POST" ? json(draft) : json({ items: [draft] });
      ...
      if (url.pathname.endsWith("/requests") && init.method === "POST") return json(requestDetail);
    ...
    await api.addTeamMember("team id", { grantId: "grant", analystId: "analyst", reason: "A sufficiently long reason." }, "token");
    await api.transferTeamMember("team id", { grantId: "grant", analystId: "analyst", reason: "A sufficiently long reason.", currentMembershipId: "membership", expectedVersion: 2, effectiveFrom: "2026-08-09T10:00:00Z" }, "token");
    await api.endTeamMembership("team id", "membership id", { grantId: "grant", expectedVersion: 2, reason: "A sufficiently long reason." }, "token");
    const calendarInput = { title: "Protected time", notes: "Required detail", startsAt: "2026-08-09T09:00:00Z", endsAt: "2026-08-09T10:00:00Z", timeZone: "Europe/London", allDay: false, category: "OTHER" as const, visibility: "PRIVATE" as const, recurrence: "NONE" as const, recurrenceInterval: 1, recurrenceUntil: null };
    ...
    await api.editCalendarOccurrence("event id", { expectedVersion: 2, occurrenceStart: calendarInput.startsAt, reason: "A sufficiently long reason.", title: calendarInput.title, notes: calendarInput.notes, replacementStart: calendarInput.startsAt, replacementEnd: calendarInput.endsAt }, "token");
    await api.splitCalendarSeries("event id", { ...calendarInput, recurrence: "DAILY", recurrenceUntil: "2026-08-12T09:00:00Z", expectedVersion: 3, splitFrom: calendarInput.startsAt, reason: "A sufficiently long reason." }, "token");
    await api.decideCalendarCommitment("event id", { expectedVersion: 1, reason: null }, true, "token");
    await api.previewTeamCapacity("team id", { grantId: "grant", dateFrom: "2026-08-01", dateTo: "2026-08-07", timeZone: "Europe/London" }, "token");
    await api.commitTeamCapacity("team id", { grantId: "grant", token: "preview-token-value-that-is-long-enough" }, "token");
    ...
    expect(productDownloadUrl("id with space")).toBe(
      "/api/v1/requests/id%20with%20space/product",
    );
  });

  it.each([
    [{ detail: { code: "AUTHENTICATION_FAILED", message: "Unable to sign in." } }, "Unable to sign in.", "AUTHENTICATION_FAILED"],
  ...
  it("announces expiry when a protected request returns 401", async () => {
    let expired = false;
    window.addEventListener(SESSION_EXPIRED_EVENT, () => { expired = true; }, { once: true });
