...
const [darkTokens, lightTokens = ""] = tokens.split(':root[data-theme="light"]');
...
function relativeLuminance(hex: string) {
  const normalised =
    hex.length === 4 ? `#${[...hex.slice(1)].map((value) => value.repeat(2)).join("")}` : hex;
...
describe("primary action contrast", () => {
  it.each([
    ...
    expect(contrast("#ffffff", token(block, "accent-strong"))).toBeGreaterThanOrEqual(4.5);
    expect(contrast("#ffffff", token(block, "accent-strong-hover"))).toBeGreaterThanOrEqual(4.5);
  });
  ...
  it("uses the checked hover token for primary buttons", () => {
    expect(base).toMatch(
      /\.button--primary:hover:not\(:disabled\)\s*\{\s*background:\s*var\(--accent-strong-hover\);\s*\}/u,
    );
  });

  it("keeps every classification strip above normal-text contrast", () => {
    const backgrounds = [...classification.matchAll(/background:\s*(#[0-9a-f]{6})/giu)].map(
      (match) => match[1],
    );
