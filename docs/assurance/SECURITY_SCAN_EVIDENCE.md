...
| Source secret scan | Pinned Gitleaks 8.30.0 scanned the dedicated source inventory, including docs and tests but excluding `.env`, caches, browser profiles and generated output; 2.70 MB scanned with no leak found |
...
| CI secret scan | Pinned TruffleHog job exists; execution awaits an initial Git baseline |
...
regression remains open upstream. CI execution still awaits the initial reviewed
Git baseline and is not represented as having run.
