...
| Gate | Objective completion condition | Required evidence | Status |
| --- | --- | --- | --- |
| PE-DOD-10 | Every representative role completes its ordinary journey from `My actions` using the same authorised results as its rollout-period queue | Positive role matrix and current Chrome, Edge and Firefox journeys for Customer, routing, Analyst, Manager, QC and Administrator roles | OPEN |
...
| PE-DOD-63 | `My actions`, notifications, product review/release, configuration, planning and statistics pass keyboard, focus, 200 per cent zoom, reduced motion, chart-table parity and 390-pixel-width review with no serious or critical axe finding | Named-page accessibility record, axe reports, screenshots and keyboard transcripts | OPEN |
