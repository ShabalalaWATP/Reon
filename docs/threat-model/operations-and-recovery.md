...
| A mutable CI service tag changes the database image without review | Pin every external workflow service image to a reviewed SHA-256 digest while retaining the human-readable tag; fail the workflow contract test when a service image lacks an exact digest | CI workflow contract test plus digest resolution for supported runner architectures |
