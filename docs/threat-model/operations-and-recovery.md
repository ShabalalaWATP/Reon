...
| A base or dependency vulnerability remains unnoticed, or a malicious new release is consumed before review | Lock dependencies, apply seven-day uv, pnpm and Dependabot version cooldowns, block exotic transitive Node sources and trust downgrades, schedule checks for every ecosystem and rerun current-database audits during release. Bootstrap exceptions are exact-version only | Frozen-install policy verification, Dependabot configuration, pip-audit, pnpm audit and image scans |
