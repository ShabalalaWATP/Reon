...
| Private event text appears in team view | Redact at repository projection before schema construction; an event's team kind never overrides its visibility, and only the authenticated subject receives concealed detail in their personal view; privacy matrix tests every view, role and recurrence form |
...
- Customer deep-link tests proving team workspaces and colleague profiles return
  to `My requests` without issuing protected team-data requests.
